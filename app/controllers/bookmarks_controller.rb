class BookmarksController < ApplicationController
  before_action :set_bookmark, only: %i[ show edit update destroy toggle_read ]
  ...
  def update
    respond_to do |format|
      if @bookmark.update(bookmark_params)
        format.html { redirect_to bookmarks_path, notice: "Bookmark was successfully updated.", status: :see_other }
  ...
  private
    ...
    def bookmark_params
      params.expect(bookmark: [ :url, :title, :description, :content, :summary, :tags, :status, :read ])
    end
end
